...
start() {
    ...
    for h in ${dns_updates[*]}; do
        ...
        [ -f $runscript ] && ( echo -e "${GREEN}=== Starting gcloud dns for $h.${mcdomain} with script ${YELLOW}$(basename $runscript) ${NC}"; as_user "$runscript start" )
    done
...
stop() {
    for h in ${dns_updates[*]}; do
        runscript=/home/sita/script/minecraft/gcloud/$h
        [ -f $runscript ] && ( echo -e "${GREEN}=== Stopping gcloud dns for $h.${mcdomain} with script ${YELLOW}$(basename $runscript) ${NC}"; as_user "$runscript stop" )
    done
